...
Assume we have an _oracle_, or "black box", that produces independent random numbers _X_ in the interval [0, 1], but with an unknown mean (average).  Assume further that the oracle's numbers are not all the same with probability 1.  Given this oracle and possibly a second source of randomness (such as unbiased random bits), are there results that give an algorithm that approximates _f_(**E**[_X_]) with a user-specified error bound, ideally using as few samples from the oracle as possible?  Here, _f_ is a known function belonging to a given class of functions, and **E**[_X_] is the true mean of the oracle's numbers.
