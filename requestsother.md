...
A _finite-state generator_ (Knuth and Yao 1976)[^2] is the special case of a pushdown generator where the probability of heads is 1/2, each digit is either 0 or 1, rules can't push stack symbols, and only one stack symbol is used. (In other words, a finite-state generator is a _finite automaton_ driven by fair coin flips.)
...
1. Suppose a finite-state generator can generate a probability distribution that takes on finitely many values. Then: Each value occurs with a rational probability; and each value is either rational or transcendental. This belief is in view of the results of Adamczewski et al. (2020)[^4].
...
- If the class is sorted permutations, both $D$ and $E$ are arbitrary distributions, and given that the return value $n$ is odd, then Forsythe (1972)[^5] and Monahan (1979)[^6] have characterized the distribution function of the sequence's first number.
...
1. Suppose $f(x)$ is continuous, and suppose $X$ is unbounded and meets additional assumptions, such as&mdash;
    ...
    or any combination of these.  Then, is there an algorithm, besides the algorithm of Kunsch et al. (2019)[^7]&mdash;
    ...
    Notice that merely having finite moments is not enough (Theorem 3.4, Kunsch et al. (2019)[^7]).  My article on [**estimation algorithms**](https://peteroupc.github.io/estimation.html) already gives a relative-error algorithm for the geometric distribution in a note.

2. Let $M_k$ be an upper bound on the $k$th central absolute moment of $X$, for $k>1$.  [**Based on the Chebyshev inequality**](https://stats.stackexchange.com/questions/555066/a-generalized-randomized-mean-estimate-based-on-the-chebyshev-inequality) (as well as Hickernell et al. (2013)[^8]; Kunsch et al. (2019)[^7]), is the mean $\mathbb{E}[X]$ within $\epsilon$ of the mean of $n$ i.i.d. samples, where&mdash;
