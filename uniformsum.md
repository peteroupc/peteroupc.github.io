...
6. If the PSRN is accepted, it fills it up with uniform random digits, then sets the PSRN's integer part to _i_, then returns the finished PSRN.  If the PSRN is not accepted, the sampler starts over from step 2.
...
6. If the bit generated in step 2 is 0 and the result of **SampleGeometricBag** is 0, fill _ret_ as in step 5, then set _ret_'s integer part to 1, then return _ret_.
7. Go to step 3.
...
4. Fill _ret_ with uniform random digits as necessary to give its fractional part the desired number of digits (similarly to **FillGeometricBag**), then set _ret_'s integer part to (1 &minus; _d_), then return _ret_
...
6. If 1 was chosen by step 2, we will sample from the right piece of the PDF, which runs along the interval [2, 3) and is a Bernstein polynomial with control points (1/2, 0, 0).  Due to the particular form of the control points, the piece can be sampled in one of the following ways:
    ...
    If _ret_ was accepted, fill _ret_ as given in step 4, then set _ret_'s integer part to 2, then return _ret_.
7. Go to step 3.
...
8. The PSRN _ret_ was accepted, so set _ret_'s integer part to _i_, then fill _ret_ with uniform random digits as necessary to give its fractional part the desired number of digits (similarly to **FillGeometricBag**), then return _ret_.
