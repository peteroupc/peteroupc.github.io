...
1. Generate an unbiased random bit, call it _d_.
2. Generate unbiased random bits until 0 is generated this way.  Set _g_ to the number of one-bits generated by this step.
3. Create an empty uniform PSRN (partially-sampled random number), call it _ret_.  Then, set the digit at position _g_ of the PSRN's fractional part to _d_ (positions start at 0 in the PSRN).
4. Fill _ret_ with uniform random digits as necessary to give its fractional part the desired number of digits (similarly to **FillGeometricBag**), and return (1 &minus; _d_) + _ret_.
...
2. If **a** has fewer digits in its fractional part than **b** (or vice versa), sample enough digits (by setting them to uniform random digits) so that both PSRNs' fractional parts have the same number of digits.  Now, let _digitcount_ be the number of digits in **a**'s fractional part.
...
4. If the base of **a**'s and **b**'s digits is 2, and if **c** is non-negative:
    1. Generate an unbiased random bit, call it _d_.
    2. Generate unbiased random bits until 0 is generated this way.  Set _g_ to the number of one-bits generated by this step.
    3. Set the digit at position (_g_ + _digitcount_) of **c**'s fractional part to _d_ (positions start at 0 in the PSRN, and digits between the positions (_digitcount_ &minus; 1) and (_g_ + _digitcount_) will be unsampled).
    4. If _d_ is 0, sum the constant 2<sup>&minus;_digitcount_</sup> into **c**.
5. Otherwise, let _base_ be that base, then apply the algorithm for sampling the sum of two uniform random numbers given earlier (except the PSRN the algorithm creates is not filled with digits after being accepted), multiply the resulting PSRN by _base_<sup>&minus;_digitcount_</sup> (which in most cases amounts to shifting the PSRN's digits "to the right" by _digitcount_ digits), and sum that PSRN into **c**.
...
3. (Now we sample the piece located at [_k_, _k_ + 1).)  Create an empty uniform PSRN, and create an input coin that returns the result of **SampleGeometricBag** on that uniform PSRN.
...
The sub-algorithm below simulates (_U_+_k_)/_base_<sup>_z_</sup>, where _U_ is the number built by the uniform PSRN, _base_ is the base (radix) of digits stored by that PSRN, _k_ is an integer 0 or greater, and _z_ is the number of significant digits in _k_ (for this purpose, _z_ is 0 if _k_ is 0).
