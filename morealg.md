...
>         - _MAYBE_ in any other case, or if the function is unsure.
>     4. **InShape** implementations often involve a shape's _signed distance field_, its _implicit curve_ or _algebraic curve_ equation (for closed curves), or its _implicit surface_ equation (for closed surfaces).
