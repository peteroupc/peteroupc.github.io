...
Begun on Mar. 5, 2016; last updated on Apr. 3, 2019.
...
- **Information security.** Keeping information safe from attacks that could access, use, delay, or manipulate that information.<sup>[**(31)**](#Note31)</sup>
...
Here, `B` can usually be calculated for different lists using the Python code in the [**appendix**](#Suggested_Entropy_Size); see also (van Staveren 2000, "Lack of randomness")<sup>[**(24)**](#Note24)</sup>.  For example, `B` is 226 (bits) for a 52-item list.  An application MAY limit `B` to 256 or greater, in cases when variety of permutations is not important.
...
- using [**hash functions**](#Hash_Functions), whose output is determined solely by the input rather than both the input and state (as with PRNGs)<sup>[**(25)**](#Note25)</sup>, and
- sampling "noise textures" with random data in each pixel (Peters 2016)<sup>[**(26)**](#Note26)</sup>.

(L'Ecuyer et al. 2015)<sup>[**(27)**](#Note27)</sup> discusses parallel generation of random numbers using PRNGs, especially on GPUs.
...
A _hash function_ is a function that takes an arbitrary input of any size (such as an array of 8-bit bytes or a sequence of characters) and returns an output with a fixed number of bits. That output is also known as a _hash code_. (By definition, hash functions are deterministic<sup>[**(28)**](#Note28)</sup>.)
...
2. Before an instance of the RNG generates a random number, it MUST have been initialized ("seeded") with a seed described as follows. The seed&mdash;
    ...
    - MAY be mixed with arbitrary data other than the seed as long as the result is no easier to predict<sup>[**(29)**](#Note29)</sup>.
...
2.  If the API implements an automatically-initialized RNG, it SHOULD NOT allow applications to initialize that same RNG with a seed for repeatable "randomness"<sup>[**(30)**](#Note30)</sup> (it MAY provide a separate PRNG to accept such a seed). See example 2.
...
<small><sup id=Note19>(19)</sup> A. Klyubin, "Some SecureRandom Thoughts", Android Developers Blog, Aug. 14, 2013.</small>

<small><sup id=Note20>(20)</sup> Michaelis, K., Meyer, C., and Schwenk, J. "Randomly Failed! The State of Randomness in Current Java Implementations", 2013.</small>

<small><sup id=Note21>(21)</sup> P. L'Ecuyer, D. Munger, et al.  "Random Numbers for Parallel Computers: Requirements and Methods, With Emphasis on GPUs". April 17, 2015.</small>
