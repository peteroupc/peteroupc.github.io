...
For random number generation purposes, good hash functions include cryptographic hash functions (e.g., SHA2-256, BLAKE2) and other hash functions that tend to produce wildly dispersed hash codes for nearby inputs, and poor hash functions include linear PRNGs such as LCGs and the Xorshift family.  Hash functions are also used for other purposes, however (including data lookup and data integrity); some hash functions are better suited for these purposes than for random number generation.<sup>[**(51)**](#Note51)</sup>
...
<small><sup id=Note51>(51)</sup> See, for example, Richter, Alvarez, Dittrich, "A Seven-Dimensional Analysis of Hashing Methods and its Implications on Query Processing", _Proceedings of the VLDB Endowment_ 9(3), 2015.</small>
