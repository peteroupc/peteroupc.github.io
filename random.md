...
- <small><sup id=Note4>(4)</sup> The definition of an RNG does not include items that generate non-uniform random numbers or signals, even if they otherwise meet the criteria of an RNG as defined here.  (For example, Gaussian and similar noise generators are not considered RNGs.)  Many of these items, however, typically serve as sources from which uniform random numbers can be derived through _randomness extraction_ techniques (see "[**Seed Generation**](#Seed_Generation)").</small>
