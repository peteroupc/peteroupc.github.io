...
The human visual system compares the responses it receives from the cones and converts them to three kinds of signals, namely a light&ndash;dark signal and the two _opponent signals_ red/green and blue/yellow. It's these signals, and not the cone responses, that are passed to the brain.<sup>[**(5)**](#Note5)</sup>
...
[**Defective color vision**](http://eilv.cie.co.at/term/287), including so-called [**"colorblindness"**](https://en.wikipedia.org/wiki/Color_blindness), can make certain kinds of light harder to distinguish than is the case with normal color vision.<sup>[**(6)**](#Note6)</sup>
...
In general, the same three numbers, such as (1, 0.5, 0.3), identify a different-appearing RGB color in different RGB color spaces.  In this document, the only RGB color space described in detail is [**sRGB**](#sRGB).  (Lindbloom)<sup>[**(8)**](#Note8)</sup> contains further information on many RGB color spaces.
...
> 2. In the TV and film industries, some RGB color spaces, including sRGB, belong in the category of so-called _standard dynamic range_ (_SDR_) color spaces, while others cover a wider range of colors (_wide color gamut_ or _WCG_), a wider "brightness" range (_high dynamic range_ or _HDR_), or both. (Mano 2018)<sup>[**(9)**](#Note9)</sup> contains an introduction to WCG/HDR images.  See also Rep. 2390-4, a more advanced overview, from the International Telecommunication Union.
...
A color string in the **HTML color format** (also known as "hex" format), which expresses 8-bpc RGB colors as text strings, consists of the character "#", two base-16 (hexadecimal) digits<sup>[**(10)**](#Note10)</sup> for the red component, two for the green, and two for the blue, in that order.
...
Other text-based color formats include the following<sup>[**(11)**](#Note11)</sup>:
...
- _Hue_ is an angle from red at 0 to yellow to green to cyan to blue to magenta to red.<sup>[**(12)**](#Note12)</sup>
...
> **Note:** The HSV color model is not perception-based, as the HWB article acknowledges<sup>[**(13)**](#Note13)</sup>.
...
> - The HSL color model is not perception-based, as the HWB article acknowledges<sup>[**(13)**](#Note13)</sup>.

<a id=HWB></a>
### HWB

In 1996, the HWB model, which seeks to be more intuitive than HSV or HSL, was published<sup>[**(13)**](#Note13)</sup>.  An HWB color consists of three components in the following order:
- _Hue_ is the same for a given RGB color as in [**HSV**](#HSV).
...
> **Note:** The HWB color model is not perception-based, as the HWB article acknowledges<sup>[**(13)**](#Note13)</sup>.
...
- Y&prime;, or _luma_, expresses an approximate "brightness".<sup>[**(14)**](#Note14)</sup>
...
- **Relative XYZ.** In this convention, the three components are divided by the luminance of a given white point.  In this case, the Y component represents a _luminance factor_; the white point has a luminance factor of 1.<sup>[**(15)**](#Note15)</sup> (In sRGB, the white point's luminance is 80 cd/m<sup>2</sup>.)
...
- For `XYZFromsRGBD50(rgb)` and  `XYZTosRGBD50(xyz)`, the white point is the D50/2 white point<sup>[**(16)**](#Note16)</sup>.
...
The CIE 1976 uniform chromaticity scale diagram is drawn using coordinates _u&prime;_ and _v&prime;_.<sup>[**(19)**](#Note19)</sup> "u&prime;v&prime;Y" form consists of _u&prime;_ then _v&prime;_  then  the Y component of an XYZ color.  "Yu&prime;v&prime;" form consists of the Y component then _u&prime;_ then _v&prime;_ of an XYZ color.
...
- _Hue_ (_h_, an angle)<sup>[**(12)**](#Note12)</sup> ranges from magenta at roughly 0 to red to yellow to green to cyan to blue to magenta.
...
The _CMYK color model_, ideally, describes the proportion of cyan, magenta, yellow, and black (K) inks to use to reproduce certain colors on a surface.<sup>[**(23)**](#Note23)</sup> However, since [**color mixture**](#Color_Mixture) of inks or other colorants is very complex, the exact color appearance of any recipe of colorants (not just in the CMYK context) depends on the _printing condition_ (as defined in ISO 12647-1), including what colorants are used, how the colorants are printed, and what surface (e.g., paper) the printed output appears on.
...
An **_encoded RGB_ color** needs to be converted to linear RGB (in the same RGB color space) before finding its luminance factor.  For example, the pseudocode below implements `Luminance(color)` for encoded sRGB colors (`LuminanceSRGB` and `LuminanceSRGBD50`)<sup>[**(16)**](#Note16)</sup>.
...
> 4. **Minimum**; e.g., `min(min(color[0], color[1]), color[2])` for three-component colors. (For techniques 1-4, see also (Helland)<sup>[**(25)**](#Note25)</sup>.)
> 5. **Light/dark factor**: A [**CIELAB**](#CIELAB) or [**CIELUV**](#CIELUV) color's lightness (_L\*_) divided by 100 (or a similar ratio in other color spaces with a light/dark dimension, such as [**HSL**](#HSL) "lightness" (Cook 2009)<sup>[**(26)**](#Note26)</sup>).
...
- `alpha`, the _alpha component_, is usually 0 or greater and 1 or less (from `color1` to `color2`), but need not be (Haeberli and Voorhees)<sup>[**(28)**](#Note28)</sup>.
...
- **Color harmonies**<sup>[**(29)**](#Note29)</sup> result by generating several colors that differ in hue (hue angle).  For each color harmony given below, the following numbers are added to a hue angle<sup>[**(12)**](#Note12)</sup> to generate the hues for the colors that make up that harmony:
...
Porter and Duff (1984) define twelve formulas for combining (compositing) two RGBA colors<sup>[**(30)**](#Note30)</sup>. In the formulas below, it is assumed that the two colors and the output are in the 0-1 format and have been _premultiplied_ (that is, their red, green, and blue components have been multiplied beforehand by their alpha component).  Given `src`, the source RGBA color, and `dst`, the destination RGBA color, the Porter&ndash;Duff formulas are as follows.
...
from 0 through 1 (the greater `s` is, the less saturated), and `r`, `g`, and `b` are as defined in the section "[**Luminance Factor (Grayscale)**](#Luminance_Factor_Grayscale)"<sup>[**(31)**](#Note31)</sup>.
- **Hue rotate.** `[-0.37124*sr + 0.7874*cr + 0.2126,  -0.49629*sr - 0.7152*cr + 0.7152, 0.86753*sr - 0.0722*cr + 0.0722, 0.20611*sr - 0.2126*cr + 0.2126, 0.08106*sr + 0.2848*cr + 0.7152, -0.28717*sr - 0.072199*cr + 0.0722, -0.94859*sr - 0.2126*cr + 0.2126, 0.65841*sr - 0.7152*cr + 0.7152, 0.29018*sr + 0.9278*cr + 0.0722]`, where `sr = sin(rotation)`, `cr = cos(rotation)`, and `rotation` is the hue rotation angle.<sup>[**(31)**](#Note31)</sup><sup>[**(32)**](#Note32)</sup>
...
> **Notes:**
>
> - For CIELAB or CIELUV, the 1976 _&Delta;E\*_<sub>ab</sub> ("delta E a b") or _&Delta;E\*_<sub>uv</sub> color difference method, respectively<sup>[**(34)**](#Note34)</sup>, is the Euclidean distance between two CIELAB or two CIELUV colors, respectively.
...
**Riemersma's method.** (Riemersma)<sup>[**(35)**](#Note35)</sup> suggests an algorithm for color difference, to be applied to **encoded RGB colors**.
...
> **Note:** An improvement to CIEDE2000 (Huang et al. 2015)<sup>[**(36)**](#Note36)</sup>, recently recommended in CIE 230:2019 for small color differences, is not yet in common use.
...
> 3. **Unique colors**: Finding the number of unique colors in an image color list can be done by storing those colors as keys in a hash table, then counting the number of keys stored this way.<sup>[**(38)**](#Note38)</sup>
...
- retrieving the name keyed to that color in a hash table (or returning an error if that color doesn't exist in the hash table)<sup>[**(38)**](#Note38)</sup>, or
...
Converting a color name to a color can be done by retrieving the color keyed to that name (or optionally, its lower-cased form) in a hash table, or returning an error if no such color exists.<sup>[**(38)**](#Note38)</sup>

If each name, number, or code in a color map is associated with one or several colors, optionally with a weighting factor for each color, then the color map is also known as a _color dictionary_ (Venn et al.)<sup>[**(42)**](#Note42)</sup>.
