...
1. The maximum number of primitives that can be shown at a time is equal to screen width times screen height divided by 24.[^62] \(See also survey project in "Other Challenges and Projects", later.)
...
- The game displays no more than 256 colors at a time.[^14]
...
- "Game X shows up to Y polygons at a time at Z frames per second and screen resolution W".[^59]
...
- A benchmark of _Quake III Arena_ averaged about 3,250 and topped out at about 6,970 triangles per frame after back-face culling, at screen resolution 640 &times; 480 (Antochi et al. 2003)[^60], (Antochi et al. 2004)[^61].
...
[^14]: This was recommended for Macintosh games in J. McCornack et al., _Tricks of the Mac Game Programming Gurus_ (Hayden Books, 1995), notably because 8-bit-per-pixel images transfer faster and save memory over images with more bits per pixel; see also chapter 70 of _Michael Abrash's Graphics Programming Black Book_ (dealing with the porting of an 8-bit-per-pixel game to a 16-bit-per-pixel video card).
