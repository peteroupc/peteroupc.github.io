...
- Screen resolutions that have been used in classic games include:[^28]
    ...
    - Game console aspect ratios: 320 &times; 224,[^36] 256 &times; 224,[^37] 256 &times; 240,[^38] 240 &times; 160,[^39] 160 &times; 144.[^40]
    - 5:4 aspect ratio:[^41] 320 &times; 256,[^42]
    - Two-color graphics: 720 &times; 348,[^43] 640 &times; 200,[^44] 512 &times; 342.[^45]
    - Enhanced Graphics Adapter aspect ratio: 640 &times; 350.[^46]
    - 8:5 aspect ratio: 640 &times; 400,[^47] 320 &times; 200.[^31]
...
- A game can support&mdash;
    ...
    or both features, without changing the size of the game's image.  For example, the original _Doom_ (1993) supported several sizes of this kind (on PC, they were 96 &times; 48, 128 &times; 64, 160 &times; 80, and so on up to 288 &times; 144, as well as 320 &times; 168 and 320 &times; 200) and optional pixel-column doubling.[^56]
...
- _Doom_ (1993) operated at 35 frames per second but could not be run at that rate (under default settings) by typical PCs of the time.[^56]
...
- The _PC 99 System Design Guide_[^60] sections 14.27 to 14.34 gives guidelines on 3-D graphics support for PCs to be launched in 1999.[^61]  This challenge recommends the writing of game software with graphics performance as good as hardware meeting such guidelines, except for the screen resolution, frame rate, and double buffering requirements.
- An application may choose to support stencil buffers, bump mapping, and three- or four-texture blending, but these are borderline pre-2000 graphics capabilities.[^62]
...
- The following are not within the spirit of this challenge:
    ...
    - Phong shading (per-pixel specular highlighting), ray tracing, and path tracing, which were too slow for real-time use in the 20th century.  Exceptions include:
        ...
        1. Per-screen-column _ray casting_, of the kind popularized in _Wolfenstein 3-D_ (1992).[^63]
...
[^30]: Nintendo 3DS lower screen; larger VGA "mode X" (256 colors); most common Nintendo 64 resolution; SEGA Mega Drive PAL.
...
[^36]: SEGA Mega Drive/SEGA Genesis NTSC; Neo Geo NTSC; effective resolution for Sega Saturn on NTSC-compatible TVs.  See also the note that follows.

[^37]: Super Famicom/Super Nintendo Entertainment System NTSC; minimum resolution of PC Engine/TurboGrafx 16.  According to Copetti's "Architecture of Consoles" series, the vertical resolution is 224 rather than 240 because the upper and lower portions of the image were hidden behind the bezel of most NTSC-compatible TVs.  As a result this is also the effective Famicom/Nintendo Entertainment System resolution for NTSC-compatible TVs.

[^38]: Famicom/Nintendo Entertainment System (but see a previous note for NTSC-compatible TVs); Super Nintendo Entertainment System PAL.

[^39]: Game Boy Advance.

[^40]: Game Boy, Game Boy Color, SEGA Game Gear.

[^41]: Aspect ratio found above all in PAL-compatible displays.

[^42]: Amiga PAL (same pixel spacing horizontally as vertically); Neo Geo PAL.

[^43]: Hercules Graphics Card two-color.

[^44]: Color/Graphics Adapter (CGA) two-color; NEC PC-8801 8-color mode; Atari ST 4-color mode.

[^45]: 12-inch classic Macintosh.

[^46]: 16 colors.  See: IBM Corp., _IBM Enhanced Graphics Adapter_, Aug. 2, 1984.

[^47]: NEC PC-9801 8-color mode; Atari ST two-color.

[^48]: Apple II.

[^49]: PlayStation Portable.

[^50]: MSX 2.

[^51]: Effective resolution of Nintendo 3DS upper screen without parallax effect.

[^52]: Virtual Boy.

[^53]: One "Tandy graphics adapter" mode.

[^54]: Minimum resolution for "handheld PCs" (_Windows CE Programmer's Guide_, MSDN Library, June 1998).

[^55]: S. Pruitt, "Frequently Asked Questions About HTML Coding for Internet Explorer 3.0", updated Jan. 30, 1997.
...
[^61]: The _PC 98 System Design Guide_, chapter 14, items 24 to 30, have largely the same guidelines.  The _PC 97 Hardware Design Guide_, chapter 15, items 24-28 and 30-31, have weaker guidelines.

[^62]: So is environment mapping, but this feature is simply the generating of texture coordinates in software for a suitably crafted texture image.  See Woo et al. (1997), chap. 9, section "Environment Mapping".

[^63]: Fabien Sanglard, [**_Game Engine Black Book: Wolfenstein 3D_**](https://fabiensanglard.net/gebb/).
