...
- Screen resolutions that have been used in classic games include 256 &times; 192, 640 &times; 480, 320 &times; 200, 320 &times; 240, 640 &times; 350, 640 &times; 200, 512 &times; 352, 512 &times; 384, and 160 &times; 120.  Classic games tended to aim for a frame rate of 30, 40, or 60 frames per second.

These limitations were inspired by the graphics limitations of classic handheld game consoles.

A game may impose further resource limits to the specifications given here (for example, to reduce the maximum number of 3D polygons, to disallow polygons, or to reduce the number of colors per tile allowed).  I would be interested in knowing about these limitations that a new game that adopts this document decides to impose.  I would also be interested in learning about a free and open-source graphics library that implements this specification.[^3]
...
- An instrument sound bank for wavetable synthesis of all instruments and drum noises in the General MIDI System level 1 specification.
    ...
    - The instrument sound bank should be in either SoundFont 2 (.sf2) or Downloadable Sounds (.dls) format. [^4]
...
[^3]: A sound bank of decent quality in either format is about 4 million bytes in size.  Making these banks would be easier if there were a guide on producing decent-quality instrument banks from the recordings of real musical instruments (rather than copying or converting other instrument banks or recording from commercial synthesizers).

[^4]: The _Multimedia PC Specification_ (1992) recommended that video cards be able to transfer up to 8-bit-per-pixel graphics at a rate of 140,000 pixels per second or faster given 40 percent of CPU bandwidth (allowing for videos of about 48 pixels square at 60 frames per second, or about 83 pixels square at 20 frames per second).  The Multimedia PC level 2 specification (1993) upped this recommendation to 1.2 million pixels per second (allowing for about 244- or 141-pixel videos at the same frame rates, respectively).  For details on these specifications, see article Q106055 in the Microsoft Knowledge Base.  Both recommendations are far from the 6.144-million-pixel fill rate needed to display 640 &times; 480 pixel video smoothly at 20 frames per second.
