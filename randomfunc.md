...
Discusses many ways applications can do random number generation and sampling from an underlying (pseudo-)random number generator and includes pseudocode for many of them.
...
All the randomization methods presented on this page are ultimately based on an underlying RNG (including a PRNG).  They assume that a generator of "truly" random numbers is available, an ideal assumption which is generally not achieved by most devices and programs in practice.  Instead, most applications make use of pseudorandom number generators (PRNGs), which are algorithms that produce _random-behaving_ numbers.  As a result, the performance and quality of the methods on this page will depend in practice on the quality of the PRNG (or other RNG) even if it doesn't in theory.  Testing the quality of an RNG is outside the scope of this document.

The randomization methods in this document will work regardless of what kind of RNG powers them (whether a PRNG, a "true" random number generator, or some other kind).  For example:

- Because PRNGs are deterministic (they produce the same values given the same state and input), if a PRNG powers any of these randomization methods, the method will likewise be deterministic and the numbers delivered by the method will at best be pseudorandom.
- If a "true" random number generator powers any of these methods, the method will not necessarily be deterministic, at least to an outside observer.
...
- [**Introduction**](#Introduction)
    - [**About Random Numbers and Random Number Generators**](#About_Random_Numbers_and_Random_Number_Generators)
...
In this document, **`RNDINT(maxInclusive)`** is the core method for using an underlying RNG to generate independent uniform random integers **in the interval [0, `maxInclusive`]**.<sup>[**(2)**](#Note2)</sup>.   There are many algorithms for `RNDINT`, but in general, the algorithm can be _unbiased_ only if it runs forever in the worst case (for further information, see "[**A Note on Integer Generation Algorithms**](https://peteroupc.github.io/randomnotes.html#A_Note_on_Integer_Generation_Algorithms)").  In the pseudocode below, which implements `RNDINT`<sup>[**(2)**](#Note2)</sup>:
...
As an example, [**"Probability and Games: Damage Rolls"**](http://www.redblobgames.com/articles/probability/damage-rolls.html) by Red Blob Games includes interactive graphics showing score distributions for lowest-of, highest-of, drop-the-lowest, and reroll game mechanics.<sup>[**(24)**](#Note24)</sup>  These and similar distributions can be generalized as follows.
...
9. **Geometric transformation:** Treat the numbers as an _n_-dimensional point, then apply a geometric transformation, such as a rotation or other _affine transformation_<sup>[**(25)**](#Note25)</sup>, to that point.
...
The pseudocode below implements an exact sampler of this distribution, with certain optimizations based on (Farach-Colton and Tsai 2015)<sup>[**(26)**](#Note26)</sup>.  Here, the parameter `p` is expressed as a ratio `px`/`py`.
...
If `successes` is a non-integer, the distribution is often called a _Pólya distribution_.  In that case, it can be sampled using the following pseudocode (Heaukulani and Roy 2019)<sup>[**(27)**](#Note27)</sup>:
...
If the probability is an irrational number, such as `exp(-x/y)` or `ln(2)`, then the digit expansion is infinite and can only be approximated.  In that case, assuming the probability is in \[0, 1), use the following algorithm to do a Bernoulli trial with that probability ((Brassard et al., 2015)<sup>[**(42)**](#Note42)</sup>; see also (Devroye 1986, p. 769)<sup>[**(10)**](#Note10)</sup>), where `BASE` is the digit base, such as 2 for binary or 10 for decimal:
...
See also (von Neumann 1951)<sup>[**(28)**](#Note28)</sup>; (Devroye 1986)<sup>[**(10)**](#Note10)</sup>, pp. 41-43; "[**Rejection Sampling**](#Rejection_Sampling)"; and "[**Generating Pseudorandom Numbers**](https://mathworks.com/help/stats/generating-random-data.html)".
...
- **`k`th-order statistic distribution**: `BetaDist(k, n+1-k)`. Returns the `k`th smallest out of `n` uniform random numbers. See also (Devroye 1986, p. 210)<sup>[**(10)**](#Note10)</sup>.
...
<small><sup id=Note17>(17)</sup> Penschuck, M., et al., "[**Recent Advances in Scalable Network Generation**](https://arxiv.org/abs/2003.00736v1)", arXiv:2003.00736v1  [cs.DS], 2020.</small>
...
<small><sup id=Note20>(20)</sup> Efraimidis, P. "[**Weighted Random Sampling over Data Streams**](https://arxiv.org/abs/1012.0256v2)", arXiv:1012.0256v2 [cs.DS], 2015.</small>
...
<small><sup id=Note23>(23)</sup> The [**Python sample code**](https://peteroupc.github.io/randomgen.zip) includes a `ConvexPolygonSampler` class that implements this kind of sampling for convex polygons; unlike other polygons, convex polygons are trivial to decompose into triangles.</small>

<small><sup id=Note24>(24)</sup> That article also mentions a critical-hit distribution, which is actually a [**mixture**](#Mixtures_of_Distributions) of two distributions: one roll of dice and the sum of two rolls of dice.</small>

<small><sup id=Note25>(25)</sup> An _affine transformation_ is one that keeps straight lines straight and parallel lines parallel.</small>

<small><sup id=Note26>(26)</sup> Farach-Colton, M. and Tsai, M.T., 2015. Exact sublinear binomial sampling. _Algorithmica_ 73(4), pp. 637-651.</small>

<small><sup id=Note27>(27)</sup> Heaukulani, C., Roy, D.M., "[**Black-box constructions for exchangeable sequences of random multisets**](https://arxiv.org/abs/1908.06349v1)", arXiv:1908.06349v1  [math.PR], 2019.  Note however that this reference defines a negative binomial distribution as the number of successes before N failures (not vice versa).</small>

<small><sup id=Note28>(28)</sup> von Neumann, J., "Various techniques used in connection with random digits", 1951.</small>
...
<small><sup id=Note79>(79)</sup> Describing differences between SQL dialects is outside the scope of this document, but [**Flourish SQL**](http://flourishlib.com/docs/FlourishSQL) describes many such differences, including those concerning RNGs.</small>
