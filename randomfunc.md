...
Begun on June 4, 2017; last updated on Apr. 6, 2019.
...
> 2. In functional programming languages such as Haskell, `RNDINT()`, as well as `RNG()` itself and other random-number-generating methods in this document, can be implemented by taking a _seed_ as an additional parameter, and returning a list of two items &mdash; the random number and a new _seed_ (as in the Haskell package `AC-Random`).  This works only if the `RNG()` implementation is deterministic.
>
> **Examples:**
>
...
In general, sorting random numbers is no different from sorting any other data. (Sorting algorithms are outside this document's scope.) (Bentley and Saxe 1980)<sup>[**(12)**](#Note12)</sup> describes a way to generate random numbers in sorted order, but it's not given here because it relies on a method akin to `RNDU01()`, which may not be adequate for large numbers of sorted random numbers depending on how `RNDU01()` is implemented.
...
> **Note:** Weighted choice _with replacement_ can be implemented by doing one or more concurrent runs of `RandomKItemsFromFileWeighted(file, 1)` (making sure each run traverses `file` the same way for multiple runs as for a single run) (Efraimidis 2015)<sup>[**(14)**](#Note14)</sup>.
...
> 2. As an extension of strategy 9 (geometric transformations), a random point (`x`, `y`) can be **rotated** to derive a point with **correlated random** coordinates (old `x`, new `x`) as follows (see (Saucier 2000)<sup>[**(18)**](#Note18)</sup>, sec. 3.8): `[x, y*sqrt(1 - rho * rho) + rho * x]`, where `x` and `y` are independent random numbers from the same distribution, and `rho` is a _correlation coefficient_ in the interval \[-1, 1\] (if `rho` is 0, the variables are uncorrelated).
>
> **Examples:**
>
...
> 4. A **compound Poisson distribution** models the sum<sup>[**(32)**](#Note32)</sup>
...
> 6. A **hypoexponential distribution** models the sum<sup>[**(33)**](#Note33)</sup>
...
4. **Stochastic interpolation** is described in (Saucier 2000)<sup>[**(18)**](#Note18)</sup>, sec. 5.3.4.  It involves choosing a data point at random, taking the mean of that point and its _k_ nearest neighbors, and shifting that mean by a random weighted sum<sup>[**(34)**](#Note34)</sup>
...
- **PDF and a uniform random variable in the interval \[0, 1\) (`randomVariable`)** are known: Create `list` and `weights` as given in the empirical sampling method, above, then divide each item in `weights` by the sum<sup>[**(35)**](#Note35)</sup>
...
> **Notes and examples:**
> - If `p` is 0.5, the binomial distribution models the task "Flip N coins, then count the number of heads.", and the return value of `Binomial` is known as [**_Hamming distance_**](https://en.wikipedia.org/wiki/Hamming_distance) (treating each trial as a "bit" that's set to 1 for a success and 0 for a failure).
...
- **Erlang distribution**: `GammaDist(n, 1.0 / lamda)`.  Expresses a sum<sup>[**(36)**](#Note36)</sup>
 of `n` exponential random variables with the given `lamda` parameter.
...
Generating N `GammaDist(total, 1)` numbers and dividing them by their sum<sup>[**(37)**](#Note37)</sup>
...
- **Bates distribution**: See [**Transformations of Random Numbers**](#Transformations_of_Random_Numbers).
...
- **Compound Poisson distribution**: See [**Transformations of Random Numbers**](#Transformations_of_Random_Numbers).
...
- **Dirichlet distribution**: See [**Random Numbers with a Given Positive Sum**](#Random_Numbers_with_a_Given_Positive_Sum).
...
- **P&oacute;lya&ndash;Aeppli distribution**: See [**Transformations of Random Numbers**](#Transformations_of_Random_Numbers).
...
To generate a random N-dimensional point on or inside an N-dimensional ball, centered at the origin, of radius R, follow the pseudocode in `RandomPointInHypersphere`, except replace `Norm(ret)` with `sqrt( S - ln(RNDU01ZeroExc()))`, where `S` is the sum of squares of the numbers in `ret`<sup>[**(39)**](#Note39)</sup>
.  For discs and spheres (2- or 3-dimensional balls), an alternative is to generate a vector (list) of N `RNDRANGE(-R, R)` random numbers<sup>[**(29)**](#Note29)</sup> until its _norm_ is R or less (see the [**appendix**](#Appendix)).<sup>[**(30)**](#Note30)</sup>
...
<small><sup id=Note5>(5)</sup> See, for example, the _Stack Overflow_ question "How to generate a number in arbitrary range using random()={0..1} preserving uniformness and density?", `questions/8019589`.</small>
...
<small><sup id=Note14>(14)</sup> Efraimidis, P. "Weighted Random Sampling over Data Streams". arXiv:1012.0256v2 [cs.DS], 2015.</small>
...
<small><sup id=Note18>(18)</sup> Saucier, R. "Computer Generation of Statistical Distributions", March 2000.</small>

<small><sup id=Note19>(19)</sup> "Jitter", as used in this step, follows a distribution formally called a _kernel_, of which the normal distribution is one example.  _Bandwidth_ should be as low or as high as allows the estimated distribution to fit the data and remain smooth.  A more complex kind of "jitter" (for multi-component data points) consists of a point generated from a [**multinormal distribution**](https://en.wikipedia.org/wiki/Multivariate_normal_distribution) with all the means equal to 0 and a _covariance matrix_ that, in this context, serves as a _bandwidth matrix_.  "Jitter" and bandwidth are not further discussed in this document.</small>

<small><sup id=Note20>(20)</sup> Other references on density estimation include [**a Wikipedia article on multiple-variable kernel density estimation**](https://en.wikipedia.org/wiki/Multivariate_kernel_density_estimation), and a [**blog post by M. Kay**](http://mark-kay.net/2013/12/24/kernel-density-estimation/).</small>
...
2. Computing the sum of twelve `RNDU01OneExc()` numbers<<|[**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.>>
 and subtracting the sum by 6 (see also [**"Irwin&ndash;Hall distribution" on Wikipedia**](https://en.wikipedia.org/wiki/Irwin%E2%80%93Hall_distribution)) results in approximate standard normal (`mu`=0, `sigma`=1) random numbers, whose values are not less than -6 or greater than 6; on the other hand, in a standard normal distribution, results less than -6 or greater than 6 will occur only with a generally negligible probability.
...
<small><sup id=Note32>(32)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note33>(33)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note34>(34)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note35>(35)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note36>(36)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note37>(37)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note38>(38)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note39>(39)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>

<small><sup id=Note40>(40)</sup> [**Kahan summation**](https://en.wikipedia.org/wiki/Kahan_summation) can be a more robust way than the na&iuml;ve approach to compute the sum of three or more numbers.</small>
