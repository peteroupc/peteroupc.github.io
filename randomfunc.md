...
Begun on June 4, 2017; last updated on Mar. 4, 2019.
...
- [**Randomization Techniques**](#Randomization_Techniques)
    ...
    - [**Randomization Applications**](#Randomization_Applications)
...
- Linear congruential generators with modulus `m`, a full period, and "good lattice structure"; a sequence of `n`-dimensional points is then `[MLCG(i), MLCG(i+1), ..., MLCG(i+n-1)]` for each integer `i` in the interval \[1, `m`\] (L'Ecuyer 1999)<sup>[**(10)**](#Note10)</sup> (see example pseudocode below).
...
<a id=Randomization_Applications></a>
...
> 6. A **hypoexponential distribution** models the sum of _n_ random numbers that follow an exponential distribution and each have a separate `lamda` parameter (see "[**Gamma Distribution**](#Gamma_Distribution)").
...
2. Use [**_rejection sampling_**](#Rejection_Sampling).  The pseudocode below generates random points in a rectangular region until a point falls within the PDF (shown as `PDF(x)` below). `minValue` and `maxValue` are the lowest and highest possible random numbers, and `maxDensity` is the maximum value of the PDF at or between those numbers.  This can be inefficient, though, if the PDF gives some numbers in the range much higher weights than it gives to others.  See also Saucier 2000, p. 39.
    ...
    If the PDF can be more easily sampled by another distribution with its own PDF (`PDF2`) that "dominates" `PDF` in the sense that `PDF2(x) >= PDF(x)` at every `x`, then generate random numbers with that distribution until a number (`n`) that satisfies `PDF(n) >= RNDRANGEMaxExc(0, PDF2(n))` is generated this way (that is, sample points in `PDF2` until a point falls within `PDF`).  `PDF2` can be multiplied beforehand by a number greater than 0 until it "dominates" `PDF`. (See also Saucier 2000, pp. 6-7; Devroye 1986, pp. 41-43; and "[**Generating Pseudorandom Numbers**](https://mathworks.com/help/stats/generating-random-data.html)".)

    For example, a custom distribution's PDF, `PDF`, is `exp(-abs(x*x*x))`, and the exponential distribution's PDF, `PDF2`, is `exp(-x)`.  The exponential PDF "dominates" the other PDF if we multiply it by 1.5, so that `PDF2` is now `1.5 * exp(-x)`.  Now we can generate numbers from our custom distribution by sampling exponential points until a point falls within `PDF`.  This is done by generating `n = -ln(RNDU01ZeroOneExc())` until `PDF(n) >= RNDRANGEMaxExc(0, PDF2(n))`.
...
<small><sup id=Note10>(10)</sup> P. L'Ecuyer, "Tables of Linear Congruential Generators of Different Sizes and Good Lattice Structure", January 1999.</small>
