...
- `numbers_from_dist_inversion` generates random numbers from a distribution via inversion (Devroye and Gravel 2018)<sup>[**(58)**](#Note58)</sup>.  `integers_from_u01` computes quantiles for a discrete distribution given its PDF.
- [**moore.py**](https://github.com/peteroupc/peteroupc.github.io/blob/master/moore.py) and `numbers_from_dist` generate random numbers from a distribution via rejection sampling (Devroye and Gravel 2016/2018)<sup>[**(65)**](#Note65)</sup>, (Sainudiin and York 2013)<sup>[**(66)**](#Note66)</sup>.
...
- **Triangular distribution**:
   - **Generalized** (Kabal 2000/2019)<sup>[**(71)**](#Note71)</sup>: `(1-alpha) * min(startpt, endpt) + alpha * max(startpt, endpt)`.  The distribution starts at `startpt` and ends at `endpt`.
...
<small><sup id=Note71>(71)</sup> Kabal, P., "Generating Gaussian Pseudo-Random Variates", McGill University, 2000/2019.</small>
...
In nearly all security-sensitive applications, random numbers generated for security purposes are integers.  In very rare cases, they're fixed-point numbers.  Even with a secure random number generator, the use of random floating-point numbers can cause security issues not present with integers or fixed-point numbers; one example is found in (Mironov 2012)<sup>[**(82)**](#Note82)</sup>.  And even in the few security applications where random floating-point numbers are used (differential privacy and lattice-based cryptography), there are ways to avoid such random numbers<sup>[**(83)**](#Note83)</sup>.
