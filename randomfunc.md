...
This section describes deriving independent uniform random integers with the help of a source of random numbers (such as a source produced by a PRNG).
...
- `NEXTRAND()` reads the next number generated by a source of random numbers (such as a source produced by a PRNG or another kind of random number generator) as described below, and
...
| If the underlying source produces: | Then `NEXTRAND()` is: | And `MODULUS` is: |
 --------- | ------ | ------ |
| Non-uniform random behaving numbers<sup>[**(81)**](#Note81)</sup>. | The next number from a new source formed by writing the underlying source's outputs to a stream of memory units (such as 8-bit bytes) and using a _randomness extraction_ technique to transform that stream to _n_-bit integers. (Note that randomness extraction is outside the scope of this document.) | 2<sup>_n_</sup>. |
...
<small><sup id=Note81>(81)</sup> An example of such a source is a Gaussian noise generator.  This kind of source is often called an _entropy source_.</small>
