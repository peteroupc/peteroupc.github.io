...
Begun on June 4, 2017; last updated on Jul. 25, 2019.
...
> 3. The implementation here uses [**_rejection sampling_**](#Rejection_Sampling) in most cases; it builds up a big number then, if it's greater than `maxInclusive`, it _rejects_ it and repeats.  This has the advantage of being unbiased but has a variable running time which could be exploited in a security attack.  An alternative approach is to find `rem(bignumber, maxInclusive + 1)`, where `bignumber` is a "big" random number (that has more bits than `maxInclusive` has).  This alternative can be "constant-time" (non-data-dependent and branchless) if implemented correctly, but can introduce a so-called _modulo bias_ (some numbers are slightly more likely to be chosen than others), which, however, gets smaller the more bits `bignumber` has.
