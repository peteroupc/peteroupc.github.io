...
2. Use [**_rejection sampling_**](#Rejection_Sampling).  The pseudocode below generates random points in a rectangular region until the point falls within the PDF (shown as `PDF(x)` below). `minValue` and `maxValue` are the lowest and highest possible random numbers, and `maxDensity` is the maximum value of the PDF at or between those numbers.  See also Saucier 2000, p. 39.
