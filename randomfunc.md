...
This section describes deriving independent uniform random integers with the help of a source of random numbers (such as one generated by a PRNG).
...
- `RNG()` reads the next number generated by an RNG described in the following table (or alternatively, reads the next number from a so-called _uniform discrete memoryless source_, that is, an endless source of random outcomes such that each outcome is equally likely and is chosen independently of any other choice).
- `MODULUS` is the number of different outcomes possible with the RNG or source.
...
3. **Stateless PRNGs.** Most designs of pseudorandom number generators (PRNGs) in common use maintain an internal state and update that state each time a random number is generated.  But for [**_stateless_ PRNG designs**](https://peteroupc.github.io/random.html#Designs_for_PRNGs) (including so-called "splittable" PRNGs), `RNDINT()`, `RNG()`, and other random sampling methods in this document may have to be adjusted accordingly (usually by adding an additional parameter).
