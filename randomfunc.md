...
Statistical analysis uses shuffling and _bootstrapping_ to help draw conclusions on data through randomization.
...
> 5. A **hypoexponential distribution** models the sum of _n_ random numbers that follow an exponential distribution and each have a separate `lamda` parameter (see "[**Gamma Distribution**](#Gamma_Distribution)").
...
2. Use [**_rejection sampling_**](#Rejection_Sampling).  The pseudocode below generates random points in a rectangular region until the point falls within the PDF (shown as `PDF(X)` below). `minValue` and `maxValue` are the lowest and highest possible random number, and `maxDensity` is the maximum value of the PDF at or between those numbers.  See also Saucier 2000, p. 39.
