...
This page discusses many ways applications can sample random content by transforming the numbers produced by an underlying source of random numbers (such as a source generated by a pseudorandom number generator \[PRNG\] or another kind of random number generator \[RNG\]) , and offers pseudocode for many of these methods. Those methods include&mdash;
...
This document uses the term "random" very broadly.
...
- `RNG()` reads the next number generated by an RNG described in the following table (or alternatively, reads the next number from a so-called _uniform discrete memoryless source_, that is, an endless source of random outcomes such that each outcome is equally likely and is generated independently of the others).
