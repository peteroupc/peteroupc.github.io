...
>
>     - choose(_g_, _g_/_t_) \* (&beta;&minus;1)<sup>_g_&minus;_g_/_t_</sup> if _g_ is divisible by _t_, and
>     - 0 otherwise,
>
...
>
> **Note:** The _square-root construction_ sqrt(1 &minus; &lambda;) (mentioned in the Flajolet paper) can be expressed as a special case of a slightly different formula from the one given here, namely&mdash;<br/>&nbsp;&nbsp;&Sigma;<sub>_k_ = 0, 1, 2, ...</sub> (&lambda;<sup>_k_</sup> * (1 &minus; &lambda;) * choose(_k_ * &alpha;, _k_) \* (&beta;&minus;1)<sup>_k_\*&alpha; &minus; _k_</sup> / &beta;<sup>_k_ * &alpha;</sup>).<br/>The square-root construction occurs if &alpha; = 2 and &beta; = 2.  Thus, the following replaces step 3 of the algorithm: "3. Generate _g_ * &alpha; uniform random integers in the interval [0, &beta;) (e.g., _g_ unbiased random bits if &beta; is 2), then return 1 if exactly _g_ zeros were generated this way, or 0 otherwise." If &beta; is 2 and &alpha; is 1, this formula simplifies to (2\*&lambda;&minus;2)/(&lambda;&minus;2). Interestingly, I have found that if &alpha; is an integer 2 or greater, the formula simplifies to involve hypergeometric functions.  Specifically, the formula becomes&mdash;
>
