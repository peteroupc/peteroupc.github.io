...
- (_&rho;_(_n_; _&lambda;_)\*V(_n_)/(_n_!)) / _p_,

where _p_ = ( &Sigma;<sub>_k_=0,1,...</sub> (_&rho;_(_k_; _&lambda;_)\*V(_k_)/(_k_!)) ), and where _&rho;_(_n_; _&lambda;_) is the probability that _G_ is _n_, with parameter _&lambda;_ or the input coin's probability of heads.  Also, the probability that _r_ random numbers are rejected by the modified algorithm is _p_*(1 &minus; _p_)<sup>_r_</sup>.

> **Example:**  If _G_ is a Poisson(1) random number and the sorted permutation class is used, the algorithm will return 0 with probability 1/_I_<sub>0</sub>(2), where _I_<sub>0</sup>(.) is the modified Bessel function of the first kind.
