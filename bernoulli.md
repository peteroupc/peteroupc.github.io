...
Certain interesting probability functions can arise from permutations, such as permutations that are sorted or permutations whose highest number appears first.  When checking whether a sequence follows a permutation, only less-than and greater-than comparisons between two numbers are allowed.
