...
To implement these probabilities using just random bits, the sampler uses algorithms from "[**Bernoulli Factory Algorithms**](https://peteroupc.github.io/bernoulli.html)".  These algorithms enable e-rands when _&lambda;_ is written as _x_/_y_ where _x_&ge;0 and _y_>0, or is written as _m_ + _&nu;_ where _m_ &ge; 0 is an integer and _&nu;_ (0 &le; _&nu;_ &le; 1) is the probability of heads of a coin.

1. To flip a coin with probability of heads of exp(&minus;_&lambda;_):
    - Use the **algorithm for exp(&minus;_x_/_y_)**.  Or...
    - Use the **algorithm for exp(&minus;(_m_ + _&lambda;_)\*_&mu;_)** where _&lambda;_ represents the coin for _&nu;_, and _&mu;_ represents a coin that always returns 1.
2. To flip a coin with probability of heads of 1/(1+exp(_&lambda;_/2<sup>_k_</sup>)) = 1 &minus; expit(_&lambda;_/2<sup>_k_</sup>):
    - Use the **1 / (1 + exp(_x_ / (_y_ * 2<sup>_prec_</sup>))** (**LogisticExp**). Or...
    - Use the **algorithm for expit((_m_ + _&lambda;_)\*_&mu;_)** where _&lambda;_ represents the coin for _&nu;_, and _&mu;_ represents a coin that returns either 1 with probability 1/(2<sup>_k_</sup>) or 0 otherwise.  However the algorithm returns 1 minus the usual result instead (leading to **1 &minus; expit((_m_ + _&lambda;_)\*_&mu;_)**).
