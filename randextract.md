...
> **Note:** In fact, the algorithm above appears to be a special case of a procedure given as a remark near the end of a paper by Camion (1974)<sup>[**(6)**](#Note6)</sup>.  Moreover, based on results by De Schuymer and others (2005)<sup>[**(7)**](#Note7)</sup>, the algorithm works regardless of what numbers _X_ and _Y_ can take on and with what probability (as long as the same "die" is used and the throws are independent), and the algorithm even works if we use two "dice" that are "loaded" differently (as long as the chance that the first "die" shows a number less than the second "die" is the same as the chance that the first "die" shows a greater number, and as long as the throws are independent).  For example, the algorithm above works even if we set _X_ and _Y_ to two independent Gaussian random numbers with the same mean and standard deviation.
