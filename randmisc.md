...
> **Note:** If the quantile function is continuous, has a minimum and maximum, and admits a modulus of continuity _&omega;_(_h_) that is continuous and monotone increasing, then _d_ in step 1 above can be calculated as&mdash;<br/>max(0, ceil(&minus;ln(_&omega;_<sup>&minus;1</sup>(_&epsilon;_))/ln(_&beta;_))),<br/>where _&omega;_<sup>&minus;1</sup>(_&epsilon;_) is the inverse of the modulus of continuity.  (Loosely speaking, a modulus of continuity _&omega;_(_h_) gives the quantile function's maximum range in a window of size _h_, and the inverse modulus _&omega;_<sup>&minus;1</sup>(_&epsilon;_) finds a window small enough that the quantile function differs by no more than _&epsilon;_ in the window.<sup>[**(28)**](#Note28)</sup>).
>
> Here is a sketch of the proof: Because the quantile function _Q_(_x_) is continuous on a closed interval [0, 1], it's uniformly continuous there.  For this reason, there is a function _&omega;_<sup>&minus;1</sup>(_&epsilon;_) such that for every _&epsilon;_&ge;0 and for any _x_ and _y_ in [0, 1], abs(_Q_(_x_)&minus;_Q_(_y_)) &lt; _&epsilon;_ whenever abs(_x_&minus;_y_) &lt; _&omega;_<sup>&minus;1</sup>(_&epsilon;_).  The inverse modulus of continuity is one such function, which is formed by inverting a modulus of continuity admitted by _Q_, as long as that modulus is continuous and monotone increasing on [0, 1] to make that modulus a one-to-one function.  Finally, max(0, ceil(&minus;ln(_z_)/ln(_&beta;_))) is an upper bound on the number of base-_&beta;_ fractional digits needed to store 1/_z_ to within _&epsilon;_.
