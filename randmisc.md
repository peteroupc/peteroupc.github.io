...
2. Calculate the quantile for G of _x_, and return that quantile. (If _x_ is a uniform PSRN, and so lies in an interval \[_a_, _b_\], this should be implemented by generating additional digits of _x_&mdash;and thus shortening the interval \[_a_, _b_\]&mdash;until a lower bound of the quantile for _a_ and an upper bound of the quantile for _b_ differ by no more than 2\*_&epsilon;_, where _&epsilon;_ is the desired accuracy, then taking the midpoint between those two bounds; see (Devroye and Gravel 2020)<sup>[**(10)**](#Note10)</sup>.)

In fact, the "X-G" families are a special case of the so-called "transformed&ndash;transformer" family of distributions introduced by Alzaatreh et al. (2013)<sup>[**(11)**](#Note11)</sup> that uses two distributions, X and G, where X (the "transformed") is an arbitrary continuous distribution, G (the "transformer") is a distribution with an easy-to-compute quantile function, and _W_ is a nondecreasing function that maps a number in [0, 1] to a number with the same support as X (and meets certain conditions to ensure the target distribution has a proper CDF).  The following algorithm samples a random number from this kind of family:
...
- The "T-R{_Y_}" family (Aljarrah et al., 2014)<sup>[**(12)**](#Note12)</sup>, in which _T_ is an arbitrary continuous distribution (X in the algorithm above), _R_ is a distribution with an easy-to-compute quantile function (G in the algorithm above), and _W_ is the quantile function for the distribution _Y_, whose support must be included in the support of _T_ (so that _W_<sup>&minus;1</sup>(_x_) is the CDF for _Y_).
- Several versions of _W_ have been proposed for the case when distribution X is supported on \[0, &infin;), such as the Rayleigh and gamma distributions.  They include:
    ...
    - _W_(_x_) = _x_/(1&minus;_x_) (_W_<sup>&minus;1</sup>(_x_) = _x_/(1+_x_)).  Suggested in the original paper by Alzaatreh et al.  This choice forms the so-called "odd X G" family, examples of which include the "odd log-logistic G" family (Gleaton and Lynch 2006)<sup>[**(13)**](#Note13)</sup> and the "generalized odd Weibull generated" family (where X is the Weibull distribution and G is arbitrary) (Korkmaz et al. 2018)<sup>[**(14)**](#Note14)</sup>.
    - Another _W_ that has been proposed is _W_(_x_) = &minus;ln(_x_) (_W_<sup>&minus;1</sup>(_x_) = exp(&minus;_x_); examples include the "Rayleigh-G" family or "Rayleigh&ndash;Rayleigh" distribution (Al Noor and Assi 2020)<sup>[**(15)**](#Note15)</sup>, as well as the "generalized gamma-G" family, where "generalized gamma" refers to the Stacy distribution (Boshi et al. 2020)<sup>[**(16)**](#Note16)</sup>), but because this _W_ is decreasing, it is strictly speaking not allowed for the "transformed&ndash;transformer" family.
...
Certain special cases of the "X-G" families use a specially designed distribution for X.  They include the following:

- The _alpha power_ or _alpha power transformed_ family (Mahdavi and Kundu 2017)<sup>[**(18)**](#Note18)</sup>. The family uses a shape parameter _&alpha;_ > 0, and the algorithm for the "X-G" families is used, except step 1 now reads: "Generate a uniform(0, 1) random number _U_, then set _x_ to ln((_&alpha;_&minus;1)\*_U_ + 1)/ln(_&alpha;_) if _&alpha;_ != 1, and _U_ otherwise."
- The _exponentiated_ family (Mudholkar and Srivastava 1993)<sup>[**(19)**](#Note19)</sup>. The family uses a shape parameter _a_ > 1; step 1 is modified to read: "Generate a uniform(0, 1) random number _U_, then set _x_ to _U_<sup>1/_a_</sup>."
- The _transmuted-G_ family (described, for example, by Tahir and Cordeiro (2016)<sup>[**(20)**](#Note20)</sup>). The family uses a shape parameter _&eta;_ in the interval [&minus;1, 1]; step 1 is modified to read: "Generate a piecewise linear random number in [0, 1] with weight 1&minus;_&eta;_ at 0 and weight 1+_&eta;_ at 1, call the number _x_."
...
An _inverse X distribution_ (or _inverted X distribution_) is generally the distribution of the reciprocal of a random number distributed as _X_.  But an _inverse exponential distribution_ (Keller and Kamath 1982)<sup>[**(22)**](#Note22)</sup> is distributed as &minus;_&theta;_/ln(_U_) where _&theta;_ > 0 and _U_ is a uniform(0, 1) random number.
...
- <small><sup id=Note10>(10)</sup> Devroye, L., Gravel, C., "[**Random variate generation using only finitely many unbiased, independently and identically distributed random bits**](https://arxiv.org/abs/1502.02539v6)", arXiv:1502.02539v6  [cs.IT], 2020.</small>
...
- <small><sup id=Note12>(12)</sup> Aljarrah, M.A., Lee, C. and Famoye, F., "On generating T-X family of distributions using quantile functions", Journal of Statistical Distributions and Applications,1(2), 2014.</small>
- <small><sup id=Note13>(13)</sup> Gleaton, J.U., Lynch, J. D., "Properties of generalized log-logistic families of lifetime distributions", _Journal of Probability and Statistical Science_ 4(1), 2006.</small>
- <small><sup id=Note14>(14)</sup> Korkmaz, M.Ç., Alizadeh, M., et al., "The Generalized Odd Weibull Generated Family of Distributions: Statistical Properties and Applications", _Pak. J. Stat. Oper. Res._ XIV(3), 2018.</small>
- <small><sup id=Note15>(15)</sup> N.H. Al Noor and N.K. Assi, "Rayleigh-Rayleigh Distribution: Properties and Applications", _Journal of Physics: Conference Series_ 1591, 012038 (2020).</small>
- <small><sup id=Note16>(16)</sup> Boshi, M.A.A., et al., "Generalized Gamma – Generalized Gompertz Distribution", _Journal of Physics: Conference Series_ 1591, 012043 (2020).</small>
...
- <small><sup id=Note18>(18)</sup> Mahdavi, Abbas, and Debasis Kundu. "A new method for generating distributions with an application to exponential distribution." Communications in Statistics-Theory and Methods 46, no. 13 (2017): 6543-6557.</small>
- <small><sup id=Note19>(19)</sup> Mudholkar, G. S., Srivastava, D. K., "Exponentiated Weibull family for analyzing bathtub failure-rate data", _IEEE Transactions on Reliability 42(2), 299-302, 1993.</small>
