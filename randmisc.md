...
> **Notes:**
>
> - A binomial(_n_, 1/2) random number, where _n_ is odd, can be generated by adding an unbiased random bit's value (either zero or one with equal probability) to a binomial(_n_&minus;1, 1/2) random number.
...
> **Note:** I suspect that if the quantile function is bounded and continuous, and admits a modulus of continuity _&omega;_(_h_) that is monotone increasing, then _d_ in step 1 above can be calculated as ceil(&minus;ln(_&omega;_<sup>&minus;1</sup>(_&epsilon;_))/ln(_&beta;_)), where _&omega;_<sup>&minus;1</sup>(_&epsilon;_) is the inverse of the modulus of continuity.  (Loosely speaking, a modulus of continuity _&omega;_(_h_) shows the quantile function's maximum range in a window of size _h_, and the inverse modulus _&omega;_<sup>&minus;1</sup>(_&epsilon;_) finds a window small enough that the quantile function differs by no more than _&epsilon;_ in the window.)
