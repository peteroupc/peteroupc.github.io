...
Note that a binomial(_n_, 1/2) random number, where _n_ is odd, can be generated by adding an unbiased random bit (zero or one) to a binomial(_n_&minus;1, 1/2) random number.
...
The first sub-algorithm returns 1 with probability (1&minus;_px_/_py_)<sup>_n_</sup>, assuming that _n_\*_px_/_py_ <= 1.
...
3. If _j_ is even, set _pnum_ to _pnum_\*_qden_ + _pden_\*_qnum_\*choose(_n_,_j_) to _pnum_. Otherwise, set _pnum_ to _pnum_\*_qden_ &minus; _pden_\*_qnum_\*choose(_n_,_j_) to _pnum_.
...
The second sub-algorithm returns an integer in [0, 2<sup>_k_</sup>) with probability (1&minus;_px_/_py_)<sup>_n_</sup>, or &minus;1 with the opposite probability.  It assumes that 2<sup>_k_</sup>\*_px_/_py_ <= 1.
