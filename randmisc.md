...
If X is a distribution supported on \[0, &infin;) \(examples include the "Rayleigh-G" family or "Rayleigh&ndash;Rayleigh" distribution (Al Noor and Assi 2020)<sup>[**(9)**](#Note9)</sup>, as well as the "generalized gamma-G" family, where "generalized gamma" refers to the Stacy distribution (Boshi et al. 2020)<sup>[**(10)**](#Note10)</sup>), the following algorithm is used instead in certain proposed distributions:

1. Generate a random number that follows the distribution X, call it _x_.
2. Calculate the quantile for G of exp(&minus;_x_), and return that quantile.

In fact, the two families just given are a special case of the so-called "transformed&ndash;transformer" family of distributions introduced by Alzaatreh et al. (2013)<sup>[**(11)**](#Note11)</sup> that uses two distributions, X and G, where X (the "transformed") is an arbitrary continuous distribution, G (the "transformer") is a distribution with an easy-to-compute quantile function, and _W_ is a function that transforms a number in [0, 1] into a number with the same support as X (and meets certain conditions to ensure a proper CDF).  The following algorithm samples a random number from this kind of family:

1. Generate a random number that follows the distribution X, call it _x_.
2. Calculate the quantile for G of _W_<sup>&minus;1</sup>(_x_) (where _W_<sup>&minus;1</sup>(.) is the inverse of _W_), and return that quantile.
...
- <small><sup id=Note9>(9)</sup> N.H. Al Noor and N.K. Assi, "Rayleigh-Rayleigh Distribution: Properties and Applications", _Journal of Physics: Conference Series_ 1591, 012038 (2020).</small>
- <small><sup id=Note10>(10)</sup> Boshi, M.A.A., et al., "Generalized Gamma – Generalized Gompertz Distribution", _Journal of Physics: Conference Series_ 1591, 012043 (2020).</small>
