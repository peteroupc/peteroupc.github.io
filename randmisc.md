...
- (7a.) Generate an exponential random number with rate 1 (which is the negative natural logarithm of a uniform(0,1) random number).  Set _&eta;_ to 0 minus that number.
...
When a random number _x_ is a uniform PSRN in \[0, 1\], then the following algorithm transforms that number to a random variate for the distribution associated with the quantile function, with a desired error tolerance of _&epsilon;_ with probability 1 (see (Devroye and Gravel 2020)<sup>[**(26)**](#Note26)</sup>):
