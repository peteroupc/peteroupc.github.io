...
Color appearance is subjective &mdash; since interpreting the light is required &mdash; and varies with the _light source_ (sunlight, daylight, incandescent light, etc.), _object_ (material), _observer_, viewing situation, or a combination of these.<sup>[(2)](#Note2)</sup>
...
The human eye has an inner back lining (called the _retina_) filled with three kinds of _cones_ (L, M, and S<sup>[(3)](#Note3)</sup>), and each kind of cone is differently sensitive to light.  The cone responses can be described by three overlapping "curves" that peak at different wavelengths of the visible spectrum &mdash; where the M and L curves span the entire visible spectrum.  As a result, at least two of the three kinds of cones will react to light, not just one by itself.
