...
- The _expectation_, _expected value_, or _mean_ is the "long-run average" value of the distribution.  It is expressed as **E**\[_X_\], where _X_ is a number taken from the stream.  If we take independent random samples and then take their average, then with probability 1, the average will approach the expected value as the number of samples gets large (the _law of large numbers_).
