...
- An _operator_ is a mapping from a function to a function.
    - An operator $L$ is _linear_ if it satisfies $L(af)=aL(f)$ and $L(f+g)=L(f)+L(g)$ for all allowed functions $f$ and $g$ and every number $a$.[^86].
...
- _$\Vert L\Vert$ is the operator norm of $L$, where the "norm" used to define the operator norm is the least upper bound on the absolute value._[^87]
...
>
>**Note:** Lemmas 11 and 12 are not guaranteed to work if $f$'s domain is a closed subset of a compact interval, unlike Lemmas 9 and 10 (see Remark 2.5 in Gonska (1998/2023)[^25]).
...
$$\text{abs}(f(\lambda)-P(\lambda))\le \frac{M (b-a)^r}{((n+1)!)\cdot 2^{2n+1}}. \tag{(Ph)}$$
>
>**Note:** The following are references on the inequality (Ph): Brass and Petras 2011[^47], example 3.1.2; Phillips 2003[^48], theorem 2.4.6.
...
In the following, given a set $S$ of functions, a subset of $S$ (which can be $S$ itself) has a "best approximation" to a function $f$ in $S$ if there is a function $g$ in the subset such that $\Vert f - g\Vert\le\Vert f - h\Vert$ for every function $h$ in the subset (De Villiers 2012, theorem 5.3.2)[^3].[^52]

**Lemma 14** (Lebesgue's lemma or Lebesgue inequality): _Let $f(\lambda)$ be a continuous function on a compact interval.  Let $T$ be a subspace[^52] of continuous functions on that interval with a "best approximation" to $f$ (such as polynomials of degree 5 or less on that interval).  Let $L$ be a linear operator such that&mdash;_
...
$$\begin{multline}\Vert f - L(f)\Vert\le(\Vert I - L\Vert)\cdot\text{Dist}(f,P)\quad\text{(Leb2)}\\\\ \le(\Vert I\Vert + \Vert L\Vert) \cdot\text{Dist}(f,P)\\\\ \le(1 + \Vert L\Vert) \cdot\text{Dist}(f,P),\quad\text{(Leb)}\end{multline}$$

_where $\Vert I-L\Vert$ is the operator norm for the difference between $f$ and $L(f)$; and $\text{Dist}(f,P)$ is the greatest lower bound of $max_t(\text{abs}(f(t)-P(t)))$ over all functions $P$ in $T$ (see also DeVore and Lorentz (1993, p. 30; ch. 5)[^53], Powell (1981, theorem 3.1)[^54], De Villiers (2012, theorem 5.3.2)[^3]; for (Leb2) see De Boor (1982, chapter 2)[^55])._[^56]
...
>
>     $$\begin{multline}\Vert f - L(f)\Vert\le(1+\Vert L\Vert )\cdot 1\cdot \left(\frac{1}{3}\right)^{3}\Vert f^{(3)}\Vert\\\\= (1+\Vert L\Vert )\Vert f^{(3)}\Vert /27.\end{multline}$$
>
...
>
>     $$\Vert f - I(f)\Vert\le\Vert I - I\Vert\cdot 0 = 0.$$
>
> **Notes:**
>
...
where $\Vert B\Vert = 1$ (De Villiers 2012, Theorem 5.2.2)[^77] is the operator norm of the Bernstein polynomials.
...
where $O(1/n^{1/2})$ is a function whose absolute value is no more than an unspecified constant times $1/n^{1/2}$. (For example, compare Sevy 1991[^26] with Gonska and Zhou 1994[^82] and Holtz et al. 2011[^75].)
...
[^77]: This includes the case that $L$ reproduces all functions it maps to (for example, if $L$ maps to polynomials up to degree 5, it reproduces all such polynomials).
