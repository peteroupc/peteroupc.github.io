...
- maps _from_ continuous or bounded functions on that interval,
- maps _to_ a finite-dimensional linear subspace of those functions (such as polynomials of degree 5 or less)[^51], and
...
where $\Vert L\Vert$ is the operator norm of $L$; and $\Vert I-L\Vert$ is the operator norm for the difference between $f$ and $L(f)$; and $\text{Dist}(f,P)$ is the greatest lower bound of $max_t(\text{abs}(f(t)-P(t)))$ over all functions $P$ mapped to by $L$ (see also DeVore and Lorentz (1993, p. 30; ch. 5)[^53], Powell (1981, theorem 3.1)[^54], De Villiers (2012, theorem 5.3.2)[^3]; for (Leb2) see De Boor (1982, chapter 2)[^55]).[^56] <!--The inequality (Leb3) follows because, if $L$ is linear and idempotent, so is $I - L$ (Rohwer 2005)[^57], and because $f - (f - L(f)) = L(f)$.-->
...
> **Notes:**
>
> 1. The only positive linear operator that reproduces all polynomials up to degree 2 (constants, linear functions, and quadratic functions) is the identity operator (Păltănea 2004, corollary 1.1.2)[^23].[^58]
...
> **Notes:**
>
> 1. An operator meeting conditions 2 and 3 is also called a _sublinear_ operator.  Every linear operator is also sublinear. A linear operator is monotone if and only if it is positive.  For more on nonlinear operators, see Gal and Niculescu (2023)[^63]; on nonlinear approximation, see DeVore (1998)[^64].)
...
- Do the [**Lebesgue inequalities**](#Lebesgue_Inequality_for_Certain_Linear_Operators) (Leb) and (Leb3) remain true when $L$ maps to _any subset_ of the functions it maps from, even if it's not a finite-dimensional linear subspace? See "[**Ways to generalize the Lebesgue inequality in approximation theory**](https://mathoverflow.net/questions/512319/ways-to-generalize-the-lebesgue-inequality-in-approximation-theory)".
- Given the work by Rohwer (2005)[^57], are the inequalities (Leb) and (Leb3) true when&mdash;
    - $L$ is nonlinear,
...
[^51]: A _linear subspace_ is a subset of another set of functions with the following property: If functions $f$ and $g$ are in the subset, so are $f+g$ and $f\cdot\alpha$ for every number $\alpha$.  If the subspace is the smallest subspace containing a given finite set of functions (such as $1$, $e_0$, and $e_1$), then it is _finite dimensional_.
...
[^57]: Rohwer, C., _Nonlinear Smoothing and Multiresolution Analysis_, Birkhäuser, 2005. [**https://doi.org/10.1007/3-7643-7382-2**](https://doi.org/10.1007/3-7643-7382-2)
