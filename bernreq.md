...
2. The following is a technical question that could help reduce this problem to the problem of finding explicit error bounds when approximating a function by polynomials.
    ...
    2. For what value of $C$ is the statement true when $W_n$ is arbitrary?  I conjecture $C = \max(2,D)$.

    3. Does the same value of $C$ suffice when $f$ has a Lipschitz continuous $(r-1)$-th derivative and $M$ is the maximum of $f$ and that derivative's Lipschitz constant?
