...
**Conjecture 1**. _A function $f$ that maps the closed interval or a subset thereof to the closed unit interval is strongly simulable if and only if&mdash;_
...
The proof follows from combining Kindler and Romik (2004, Theorem 2)[^59] and Knuth and Yao (1976)[^37] with Richman (2012)[^64], who proved that a continuous algebraic function on an open interval is piecewise analytic ("analytic" means writable as $c_0 x^0+ ... +c_i x^i + ...$ where $c_i$ are real numbers).
