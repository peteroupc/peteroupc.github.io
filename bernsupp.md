...
**Lemma B11:** Let $H_{n,r}(f)$ be the function described in (Han 2003)[^39], equation 16, with $a=0$ and $b=1$, and let $f(\lambda)$ have a continuous $(2+r)$-th derivative on the closed unit interval, where $r\ge 2$ is an integer.  Suppose $r$ is even.  Then the error in approximating $f$ is bounded as follows: $$|H_{n,r}(f)(\lambda) - f(\lambda)|\le M\beta\frac{1}{(r+1)\cdot((z/2+1)!)\cdot (6n)^{r/2+1}},$$ where $\beta$ is 2 if $r$ is odd, and 1 otherwise, and where $M$ is the maximum of the absolute value of $f$'s $(2+r)$-th derivative.
