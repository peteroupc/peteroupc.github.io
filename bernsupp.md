...
**Fourth scheme.**  In this scheme, which relates to a result from Kopotun et al. (2017)<sup>[**(8)**](#Note8)</sup>, let _f_ be a nondecreasing and Lipschitz continuous function in \[0, 1\].  Then for all _n_&ge;2:
...
|   Property of simulation   |   Property of _f_
  ------------- |  ------------------------
| Requires no more than _n_ input coin flips. | If and only if _f_ can be written as a polynomial in Bernstein form of degree _n_ with coefficients in \[0, 1] (Goyal and Sigman 2012)<sup>[**(9)**](#Note9)</sup>. |
...
The following note shows the complexity of the algorithm for 1/_&omega;_ in the main article, where _&omega;_ is the golden ratio.

Let **E**\[_N_\] be the expected (average) number of unbiased random bits (fair coin flips) generated by the algorithm.

Then, since each bit is independent, **E**\[_N_\] = 2\*_&omega;_ as shown below.

- Each iteration stops the algorithm with probability _p_ = (1/2) + (1&minus;(1/2)) * (1/_&omega;_) (1/2 for the initial bit and 1/_&omega;_ for the recursive run; (1&minus;(1/2)) because we're subtracting the (1/2) earlier on the right-hand side from 1).
...
- Each iteration uses 1 * (1/2) + (1 + **E**\[_N_\]) * (1/2) bits on average, so the whole algorithm uses **E**\[_N_\] = (1 * (1/2) + (1 + **E**\[_N_\]) * (1/2)) * **E**\[_T_\] bits on average (each iteration consumes either 1 bit with probability 1/2, or (1 + **E**\[_N_\]) bits with probability 1/2). This equation has the solution **E**\[_N_\] = 1 + sqrt(5) = 2\*_&omega;_.

Also, on average, half of these flips (_&omega;_) show 1 and half show 0, since the bits are unbiased (the coin is fair).
...
- <small><sup id=Note8>(8)</sup> Kopotun, K.A., et al., "[**Interpolatory pointwise estimates for monotone polynomial approximation**](https://arxiv.org/abs/1711.07083)", arXiv:1711.07083 [math.CA], 2017.</small>
...
1. _Let &omega;(x) be a modulus of continuity of f (a non-negative and nondecreasing function in the interval [0, 1], for which abs(f(x) &minus; f(y)) &le; &omega;(abs(x&minus;y)) for all x in [0, 1] and all y in [0, 1]).  If &omega; is concave on [0, 1], then the expression&mdash;<br>abs(**E**[f(X/n)] &minus; f(k/(2\*n))),&nbsp;&nbsp;&nbsp;(1)<br>is bounded from above by&mdash;_
...
> 5. All continuous functions that map [0, 1] to [0, 1] have a modulus of continuity.  If _f_ is nondecreasing and concave, and equals 0 at 0, then _f_ is also its own modulus of continuity.
...
_Proof._ Follows from Theorem 1 and the definition of Lipschitz-continuous functions by using _&omega;_(_x_) = _M_\*_x_ (this function is a concave modulus of continuity under part 1 of Lemma 2a). This special case of Theorem 1 was already found by Nacu and Peres (2005)<sup>[**(1)**](#Note1)</sup>. &#x25a1;
