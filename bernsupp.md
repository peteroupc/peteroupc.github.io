...
[^29]: Gal, S.G., "Calculus of the modulus of continuity for nonconcave functions and applications", _Calcolo_ 27 (1990)

[^30]: Gal, S.G., 1995. Properties of the modulus of continuity for monotonous convex functions and applications. _International Journal of Mathematics and Mathematical Sciences_ 18(3), pp.443-446.
...
[^61]: Vatutin, V.A., Mikhailov, V.G., "Limit theorems for the number of empty cells in an equiprobable scheme for group allocation of particles", Theory Probab. Appl. 27 (1982).

[^62]: Hoeffding, W., "Probability inequalities for sums of bounded random variables", Journal of the American Statistical Association 58 (1963).

[^63]: Adell, J.A., Cárdenas-Morales, D., "[**Quantitative generalized Voronovskaja’s formulae for Bernstein polynomials**](https://www.sciencedirect.com/science/article/pii/S0021904518300376)", Journal of Approximation Theory 231, July 2018.
...
2. _If f is Hölder continuous with Hölder constant M and with Hölder exponent &alpha; such that 0 < &alpha; &le; 1, then the expression (1) is less than or equal to&mdash;
    - _M\*(1/(2\*n))$^{\alpha/2}$,_
...
3. See Lemma 2A, later, with the random variable $X/n$ in part 1, and plugging in estimates for the variance of $X/n$ found in part 1.
...
_where $r = (2-2\alpha)/(1+\alpha)$.  If $\alpha$ is 1 (so that $f$ is Lipschitz continuous), then instead&mdash;_
...
> **Note:** The last condition of Proposition 3, condition (ii), says **fabove**(_n_, _k_)\*choose(_n_,_k_) and **fbelow**(_n_, _k_)\*choose(_n_,_k_) must be integers. [^37] But Proposition 3 assumes only the biased coin and no other randomness is used, and that the coin doesn't show heads every time or tails every time.  Therefore, _f_(0), if it exists, must be an integer, and the same is true for _f_(1), so that condition (ii) is redundant with condition (iii) due to a result that goes back to Kantorovich (1931)[^38]; see also Remark C of Nacu and Peres (2005)[^1].
...
The following lemma on hypergeometric random variables is useful.

**Lemma C1.** Let $X$ be a hypergeometric($2n$,$k$,$n$) random variable.  Denote the $s$-th absolute central moment of $X$ as $\tau_s(X)$.  Then, for every even integer $s\ge 0$ and every integer $0\le k\le n$:

$$\tau_s(X) \le \frac{s!}{8^{s/2}((s/2)!)n^{s/2}},$$

_Proof_: It is known that, if $s$ is an even integer, the $s$-th central moment of any random real-number variable, including $X$, is equal to the $s$-th absolute central moment.

The $s$-th central moment of $X$ is the expected or "long-range" value of a function of $X$, namely $(X - k/2)^s$.  This function is a convex function in X whenever $s\ge 0$ is an even integer and $0\le X\le k$, and hypergeometric random variables are so-called _Poisson binomial_ random variables (Vatutin and Mikhailov 1982)[^61], so by Hoeffding (1963)[^62], this central moment is bounded above by the $s$-th central moment of $Y$, a binomial random variable expressing the number of "successful" samples among $k$ independent samples with "success" probability 1/2; $Y$ has the same mean as $X$, namely $k/2$.

In turn, a result of Adell and Cárdenas-Morales (2018)[^63] gives an upper bound of $\tau_s(Y)$ for even $s$.
...
**Corollary C2.** Let $X$ be a hypergeometric($2n$,$k$,$n$) random variable.  Denote the $s$-th absolute central moment of $X$ as $\tau_s(X)$.  Then, for every integer $0\le k\le n$, $\tau_4(X)\le \frac{3}{16}n^2$.
...
By Corollary C2, $\tau_4(X) \le \frac{3}{16}n^2$.

The well-known fact that $\tau_3(Y)\le (\tau_4(Y))^{3/4}$ whenever the random variable $Y$ has a fourth moment gives:
