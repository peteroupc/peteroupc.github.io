...
- A _concave_ function $f$ has the property that if $f((x+y)/2) \ge (f(x)+f(y))/2$ whenever $x$, $y$, and $(x+y)/2$ are in the domain of $f$.<br>Roughly speaking, if $f$'s "slope" never goes up, then it's concave.
