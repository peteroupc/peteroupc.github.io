...
**Lemma 2A.**  _Let f map the closed unit interval to itself and let X be as in Lemma 2.  If f has a continuous second derivative whose absolute value has a maximum of M or greater, then the expression (1) is less than or equal to M\*3/(8\*n), for every integer n&ge;1_.
...
Given that _f_ has a continuous second derivative, it satisfies $\omega_{2}(f, h)\le M h^2$ whenever $0\le h\le 1/2$ (which is the case here due to the domain of _f_ and the definition of $\omega_{2}$) (Timan 1994)[^45].
...
The proof follows from combining Kindler and Romik (2004, Theorem 2)[^60] and Knuth and Yao (1976)[^37] with Richman (2012)[^65], who proved that a continuous algebraic function on an open interval is piecewise analytic ("analytic" means writable as $c_0 x^0+ ... +c_i x^i + ...$ where $c_i$ are real numbers).
