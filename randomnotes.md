...
<small><sup id=Note22>(22)</sup> Oberhoff, Sebastian, "[**Exact Sampling and Prefix Distributions**](https://dc.uwm.edu/etd/1888)", _Theses and Dissertations_, University of Wisconsin Milwaukee, 2018.</small>

<small><sup id=Note23>(23)</sup> A.J. Walker, "An efficient method for generating discrete random variables with general dlstributions", _ACM Transactions on Mathematical Software_ 3, 1977.</small>

<small><sup id=Note24>(24)</sup> Vose, Michael D. "A linear algorithm for generating random numbers with a given distribution." IEEE Transactions on software engineering 17, no. 9 (1991): 972-975.</small>

<small><sup id=Note25>(25)</sup> Feras A. Saad, Cameron E. Freer, Martin C. Rinard, and Vikash K. Mansinghka. Optimal Approximate Sampling From Discrete Probability Distributions. Proc. ACM Program. Lang. 4, POPL, Article 36 (January 2020), 33 pages.</small>
...
Just like integer generation algorithms (see the previous sections), weighted choice algorithms (implementations of `WeightedChoice`) involve generating random integers with separate probabilities.  And all of them can be described as a binary DDG tree just like integer generating algorithms.
...
- The _alias method_ by Walker (1977)<sup>[**(23)**](#Note23)</sup>.  Michael Vose's version of the alias method (Vose 1991)<sup>[**(24)**](#Note24)</sup> is described in "[**Darts, Dice, and Coins: Sampling from a Discrete Distribution**](https://www.keithschwarz.com/darts-dice-coins/)".
- The samplers described in (Saad et al., 2020)<sup>[**(25)**](#Note25)</sup>.   The samplers are entropy-optimal as long as the sum of the weights is of the form 2<sup>k</sup> or 2<sup>k</sup> &minus; 2<sup>m</sup>.
- The Fast Loaded Dice Roller (Saad et al., 2020)<sup>[**(26)**](#Note26)</sup>.  This sampler comes within 6 bits, on average, of the optimal number of bits.
- The alias method and other data structures were surveyed and mentioned in (Klundert 2019)<sup>[**(27)**](#Note27)</sup>.
- The Bringmann&ndash;Larsen succinct data structure (Bringmann and Larsen 2013)<sup>[**(28)**](#Note28)</sup>.
- A version of the alias method that works on sorted weights (Bringmann and Panagiotou 2012)<sup>[**(29)**](#Note29)</sup>.
...
- The Knuth and Yao algorithm that generates a DDG tree from the binary expansions of the probabilities.  This is suggested in exercise 3.4.2 of chapter 15 of (Devroye 1986, p. 1-2)<sup>[**(10)**](#Note10)</sup>, implemented in _randomgen.py_ as the `discretegen` method, and also described in (Roy et al. 2013)<sup>[**(31)**](#Note31)</sup>.
- The Han and Hoshi algorithm (Han and Hoshi 1997)<sup>[**(32)**](#Note32)</sup> that uses the cumulative probabilities as input and is described in (Devroye and Gravel 2015)<sup>[**(21)**](#Note21)</sup>.
