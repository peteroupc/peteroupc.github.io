...
2. An _error-bounded algorithm_ is a sampling algorithm with the following requirements:
    ...
    - If the ideal distribution is discrete (takes on values that can map to integers and back without loss), the algorithm samples exactly from that distribution. (But see the note later.)
